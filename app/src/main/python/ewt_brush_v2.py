...
async def run_brush_all(
    ...
    try:
        ...
        # 指定课时：支持逗号分隔多值（App 队列刷：lesson_filter="id1,id2,..." 一次并行刷多课）
        if lesson_filter:
            want = {str(x).strip() for x in str(lesson_filter).split(",") if str(x).strip()}
            tasks = [t for t in tasks if str(t[1].get("lessonId")) in want]
        if not tasks:
            print("\n没有未完成的课时（可能已全部刷完）")
            return 0
        if offset or limit:
